...
private:
    ...
    VectorXd _Jdot_qdot; // Xdot = Jdot * qdot matrices of the panda
    ...
    ////////////// OSC ////////////
    Vector3d goal_pos, goal_ori; 
    VectorXd goal_;   
    MatrixXd _J_T_hands; // Transpose jacobian matrix
    MatrixXd _J_barT_hands; // pseudo inverse transpose jacobian matrix
    MatrixXd _Null; // null space projection matrix
    MatrixXd _lambda; // task space inertia matrix
    VectorXd _F_star; // first task
    VectorXd _second_task; // second task (redundant null)
    VectorXd _q_des0;
    int j;
    bool plan_b;
    double _kps;
